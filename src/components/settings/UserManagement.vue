<template>
    <div>
        ...
        <div v-if="!isAdmin" class="alert alert-warning">
            {{ $t("Only the admin user can manage users. You can view the user list but cannot make changes.") }}
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="form-text text-muted">{{ $t("Password must be at least 8 characters long") }}</small>
                        </div>
                        <div class="mb-3">
...
export default {
    ...
    methods: {
        ...
        addUser() {
            if (!this.isAdmin || !this.canAddUser) return;
            
            this.$root.getSocket().emit("addUser", this.newUser.username, this.newUser.password, (res) => {
                if (res.ok) {
                    useToast().success(res.msg || "User added successfully");
        ...
        changePassword() {
            if (!this.canChangePassword || !this.selectedUser || !this.canManageUser(this.selectedUser)) return;
            
            this.$root.getSocket().emit("changeUserPassword", this.selectedUser.id, this.passwordChange.newPassword, (res) => {
                if (res.ok) {
                    useToast().success(res.msg || "Password changed successfully");
        ...
        deleteUser() {
            if (!this.selectedUser || !this.canManageUser(this.selectedUser)) return;
            
            this.$root.getSocket().emit("deleteUser", this.selectedUser.id, (res) => {
                if (res.ok) {
                    useToast().success(res.msg || "User deleted successfully");
        ...
        toggleUserActive(user) {
            ...
            this.$root.getSocket().emit("toggleUserActive", user.id, (res) => {
                if (res.ok) {
                    useToast().success(res.msg || (user.active ? "User deactivated" : "User activated"));
                    this.loadUsers();
                } else {
                    useToast().error(res.msg);
                }
